...
void Shader2::Load()
{
	...
	programs[PROGRAM_COMPLEX_NORMALMAP_LAVAGLOW] = glCreateProgram();
		...
		shader = glCreateShader(GL_FRAGMENT_SHADER);
		load_shader(shader,"Shaders/glow.frag");
		glCompileShader(shader);
		glAttachShader(programs[PROGRAM_COMPLEX_NORMALMAP_LAVAGLOW], shader);
	...
	programs[PROGRAM_ASTEROID] = glCreateProgram();
	shader = glCreateShader(GL_VERTEX_SHADER);
	load_shader(shader, "Shaders/simple.vert");
	glCompileShader(shader);
	glAttachShader(programs[PROGRAM_ASTEROID], shader);

	shader = glCreateShader(GL_FRAGMENT_SHADER);
	load_shader(shader, "Shaders/textured.frag");
	glCompileShader(shader);
	glAttachShader(programs[PROGRAM_ASTEROID], shader);
	glLinkProgram(programs[PROGRAM_ASTEROID]);


	glDeleteShader(shader);
}

void Shader2::Activate(int program_id)
{
	glUseProgram(programs[program_id]);
	current_prog_id = program_id;
}

void Shader2::Deactivate()
{
	glUseProgram(0);
}